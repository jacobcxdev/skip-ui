...
//
// Vendored from Calvin-LL/Reorderable (Apache 2.0)
 ...
 *
 * @param lazyListState The return value of rememberLazyListState
 * @param scrollThresholdPadding Padding for scroll threshold calculation
 * @param scrollThreshold Distance from edge that triggers scrolling
 * @param scroller The [Scroller] used to scroll the list during drag
 * @param onMove Called when an item is moved — must return only after items are moved
 */
@Composable
...
class ReorderableLazyListState internal constructor(
    ...
    listState.toLazyCollectionState(),
    scope,
    onMoveState,
    scrollThreshold,
    scrollThresholdPadding,
 ...
 *
 * This is a standalone version (not a LazyItemScope extension) for Skip/Swift compatibility.
 * The caller must provide [animateItemModifier] explicitly — there is no default animateItem().
 *
 * @param state The return value of [rememberReorderableLazyListState]
 * @param key The key of the item, must match the key passed to items()
 * @param modifier Modifier applied to all items
 * @param enabled Whether this item is reorderable
 * @param animateItemModifier Modifier for items that are NOT being dragged (typically animateItem())
 * @param content Content lambda receiving [ReorderableCollectionItemScope] and isDragging
 */
@Composable
fun ReorderableItem(
    ...
    animateItemModifier: Modifier = Modifier,
